...
SELECT ${ALL_EMPLOYEES_COLUMNS.map(ename => `e.${ename}`).join(',')}, count(*) as ordercount
FROM Employee as e join CustomerOrder as co On e.id = co.employeeid group by e.id`);
}
